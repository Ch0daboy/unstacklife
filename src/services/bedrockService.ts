...
const AWS_REGION = 'us-east-1'; // Default region, can be configured
const BEDROCK_RUNTIME_URL = `https://bedrock-runtime.${AWS_REGION}.amazonaws.com`;
...
const createSignature = async (
  ...
  // This is a simplified implementation - you would typically use AWS SDK
  // For production, recommend using @aws-sdk/client-bedrock-runtime
  const encoder = new TextEncoder();
  const key = await crypto.subtle.importKey(
    'raw',
    encoder.encode(credentials.secretAccessKey),
...
const callBedrockAPI = async (
  prompt: string,
  credentials: BedrockCredentials,
  modelId: string = BEDROCK_MODELS.PRIMARY,
  maxTokens: number = 2048,
  isCancelledFn?: () => boolean
): Promise<string> => {
  const maxRetries = 3;
  const region = credentials.region || AWS_REGION;
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    ...
    try {
      ...
      // For production, use AWS SDK instead of manual signing
      const url = new URL(`${BEDROCK_RUNTIME_URL}/model/${modelId}/invoke`);
      
      const headers = {
        'Content-Type': 'application/json',
        'Authorization': `AWS4-HMAC-SHA256 Credential=${credentials.accessKeyId}/${new Date().toISOString().slice(0,10)}/${region}/bedrock/aws4_request`,
      ...
      } else if (data.completions && data.completions[0]?.data?.text) {
      ...
      if (attempt >= maxRetries) {
        console.error('Bedrock API failed after all retries:', error);
        throw error;
      }
...
`;
  ...
  const response = await callBedrockAPI(fullPrompt, credentials, BEDROCK_MODELS.PRIMARY);
  ...
  try {
    ...
    return {
      ...
      status: 'draft',
      chapters: bookData.chapters.map((chapter: any) => ({
        id: uuidv4(),
        title: chapter.title,
    ...
    throw new Error('Failed to parse book outline from AI response. Please check your credentials and try again.');
  }
};
...
`;
  ...
  const response = await callBedrockAPI(prompt, credentials, BEDROCK_MODELS.PRIMARY);
  ...
  try {
    ...
    return outlineData.sections.map((section: any) => ({
      id: uuidv4(),
      title: section.title,
...
`;
  ...
  const response = await callBedrockAPI(prompt, credentials, BEDROCK_MODELS.PRIMARY, 3000, isCancelledFn);
...
export const generateContentWithHeatLevel = async (
  ...
  if (perspective) {
    const perspectiveDescriptions = {
      ...
      'third-omniscious': 'Write in third person omniscient narrative (using "he/she" perspective), with access to multiple characters\' thoughts.',
...
`;
  ...
  const response = await callBedrockAPI(prompt, credentials, BEDROCK_MODELS.PRIMARY, 3000);
  return response.trim();
};
